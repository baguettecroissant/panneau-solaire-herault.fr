 ...
 * 
 * Target: panneau-solaire-herault.fr (Hérault 34)
 */

// ── Site-specific config ──
const SITE_DOMAIN = 'panneau-solaire-herault.fr';
...
// ── Category name mapping for Solaire ──
const CAT_NAMES = {
...
export async function onRequestPost(context) {
  try {
    ...
    const {
      ...
      catId = 37, typeBien, situation, chauffageActuel, delais,
      pageUrl,
    } = rawBody;
    ...
    if (!cp || !CP_PATTERN.test(cp)) errors.push(`Code postal invalide (doit être un code postal de l'Hérault commençant par 34)`);
    ...
    const workDescription = `Projet: ${catName} à ${ville} (${cp}). Configuration: ${chauffageActuel || 'Non renseigné'}. Délai souhaité: ${
    ...
    if (code === '200') {
      ...
      return new Response(JSON.stringify({
        success: false,
        errors: vudErrors.length > 0 ? vudErrors : ['Le partenaire n\'a pas validé la demande.'],
